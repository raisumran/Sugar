...
$mod_strings = array(
    ...
    'LBL_DEFAULT_SUBPANEL_TITLE' => '高収益商品',
    ...
    'WARNING_DELETED_RECORD_RECOMMIT_2' => 'これは削除され、再度コミットする必要があるようになります',
    ...
    'LBL_HELP_CREATE' => '{{plural_module_name}}モジュールは、{{opportunities_singular_module}}のレコードに含まれており、販売のライフサイクルを通じて追跡しているビジネスの個々の製品で構成されています。{{opportunities_singular_module}}レコードが1つ以上の{{module_name}}レコードのヘッダーとして機能します。 {{module_name}}を作成するには、1.必要に応じてフィールドの値を指定します。 - 「必須」フィールドは保存前に入力完了しなければなりません。- 必要ならば、追加のフィールドを表示する「詳細を表示」をクリックしてください。2.新しいレコードを確定し、前のページに戻るには「保存」をクリックします。「保存して表示」を選択してレコード表示の新しい{{module_name}}を開いてください。「保存して新規作成」を選択して新しい{{module_name}}をすぐに作成します。',
