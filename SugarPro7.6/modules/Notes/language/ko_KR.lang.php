...
$mod_strings = array(
    ...
    // Help Text
    // List View Help Text
    'LBL_HELP_RECORDS' => '{{plural_module_name}} 모듈은 텍스트 또는 관련 기록과 관련된 첨부 파일을 포함하는 개별 {{plural_module_name}}으로 구성되어 있습니다. 모듈 _ {{}} 레코드 플렉스를 통해 대부분의 모듈에 하나의 레코드와 관련 될 수는 필드 관해서 또한 단일 contacts_singular_module {{}} 관련 될 수있다. {{}} plural_module_name 레코드 또는 레코드에 관련도 첨부 파일에 대한 일반적인 텍스트를 보유 할 수 있습니다.{{모듈 _}} 레코드가 만들어지면 등, 역사의 서브 패널을 통해 {{plural_module_name을}} 오기 같은 {{plural_module_name}} 모듈을 통해 같은 설탕에 당신이 {} {plural_module_name을} 만들 수있는 다양한 방법, 당신이 있습니다{{}} plural_module_name 기록보기를 통해 {{모듈 _}}에 관한 정보를보고 편집 할 수 있습니다. 각 모듈 _ {{}} 기록은 다음과 같은 {{accounts_module}} {{contacts_module}} {{opportunities_module}}, 그리고 많은 다른 사람과 같은 다른 설탕 레코드와 관련 될 수있다.',
    ...
    'LBL_HELP_CREATE' => '{} {모듈 _}를 만들려면. 1 원하는 필드의 값을 제공합니다. - 필드는 "필수"로 표시하기 전에 저장을 완료해야합니다. - 필요한 경우 추가 필드를 노출 "더보기"를 클릭합니다. 2. 새 레코드를 완료하고 이전 페이지로 돌아갑니다 "저장"을 클릭합니다. - 선택 "저장 및보기"레코드보기에 {} {모듈 _} 새를 엽니 다. - 선택 "저장 및 새로 작성"바로 또 다른 새로운 만들 수 {{모듈 _}}을.',
);
