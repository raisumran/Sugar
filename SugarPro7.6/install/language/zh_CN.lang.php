...
$mod_strings = array(
    ...
    'LBL_OC_SUCCESS'                    => '离线客户端安装完成。请单击下方按钮转到您的系统。',
    ...
    'ERR_ADMIN_USER_NAME_BLANK'         => '请提供Sugar管理员用户的用户名。',
	'ERR_ADMIN_PASS_BLANK'				=> '请提供Sugar管理员用户的密码。',

    'ERR_CHECKSYS'                      => '在兼容性检查过程中发现错误。为了确保您的SugarCRM可以正常工作，请采取正确步骤解决下列问题并单击再次检查按钮或再次执行安装。',
    'ERR_CHECKSYS_CALL_TIME'            => 'Allow Call Time Pass Reference是开启的(php.ini中的这个选项应设置为OFF)',
	'ERR_CHECKSYS_CURL'					=> '未找到：Sugar定时任务只在有限功能情况下运行。电子邮件归档服务不运行。',
    'ERR_CHECKSYS_IMAP'					=> '未找到：接收邮件以及市场活动(邮件)需要IMAP库。否则不能正常工作。',
	'ERR_CHECKSYS_MSSQL_MQGPC'			=> '若使用MS SQL数据库，Magic Quotes GPC不可设置为"On"。',
	'ERR_CHECKSYS_MEM_LIMIT_0'			=> '警告：',
	'ERR_CHECKSYS_MEM_LIMIT_1'			=> '(您的php.ini应设置此项为',
	'ERR_CHECKSYS_MEM_LIMIT_2'			=> 'M或更大)',
	'ERR_CHECKSYS_MYSQL_VERSION'		=> '至少必须使用版本4.1.2 - 找到：',
	'ERR_CHECKSYS_NO_SESSIONS'			=> '读取或写入会话变值失败。不能继续此次安装。',
	...
	'ERR_CHECKSYS_FASTCGI'              => '我们检测到您目前没有使用FastCGI处理器来映射PHP。您需要安装/配置一个和Sugar兼容的版本。请参考发布笔记中的兼容列表来寻找支持的版本。请参考<a href="http://www.iis.net/php/" target="_blank">http://www.iis.net/php/</a>以便获得更多信息',
    ...
    'LBL_CHECKSYS_DB_SUPPORT_NOT_AVAILABLE' => '数据库支持找不到。请确保下列可支持的数据库必要的驱动器已安装：MySQL、MS SQLServer、Oracle或DB2。您可能要移除php.ini文件中有关扩展的注释，或重新编译正确的二进制文件，视您的PHP版本而定。关于如何启动数据库支持，请参考您的PHP手册。',
    'LBL_CHECKSYS_XML_NOT_AVAILABLE'        => '找不到Sugar应用程序需要的XML解析库功能。您可能要移除php.ini文件中有关扩展的注释，或重新编译正确的二进制文件，视您的PHP版本而定。欲知详情，请参考您的PHP手册。',
    ...
    'ERR_CHECKSYS_SESSION_SAVE_PATH_NOT_SET'       => '您的PHP配置文件(php.ini)中的session.save_path未设置或设置到不存在的目录。您需要在php.inin中设置save_path或确定php.ini中所设置的save_path文件夹存在。',
    'ERR_CHECKSYS_SESSION_SAVE_PATH_NOT_WRITABLE'  => '您的PHP配置文件(php.ini)中的session.save_path文件夹不可写入。请采取必需步骤将文件夹设置为可写入。<br>这可能需要您运行chmod 766来变更权限或按右键单击文件名查看属性并取消只读选项，视您所用操作系统而定。',
    'ERR_CHECKSYS_CONFIG_NOT_WRITABLE'  => '配置文件存在但不可写入，请确保这个文件可写入。这可能需要您运行chmod 766来变更权限或按右键单击文件名查看文件名并取消只读选项，视您所用操作系统而定。',
    'ERR_CHECKSYS_CONFIG_OVERRIDE_NOT_WRITABLE'  => '存在设置重载文件但不可写入。请采取必要步骤使文件可写入。这可能需要您运行chmod 766来变更权限或按右键单击文件名查看属性并取消只读选项，视您所用操作系统而定。',
    'ERR_CHECKSYS_CUSTOM_NOT_WRITABLE'  => '自定义文件存在，但不可写入。请采取必要步骤使文件可写入。这可能需要您运行chmod 766来变更权限或按右键单击取消只读选项，视您所用操作系统而定。',
    'ERR_CHECKSYS_FILES_NOT_WRITABLE'   => "下列文件或目录不可写入或找不到并且不能创建。这可能需要您运行chmod 755来变更权限或按右键单击父目录取消包含子目录在内的目录树的只读选项，视您所用操作系统而定。",
	'ERR_CHECKSYS_SAFE_MODE'			=> 'Safe Mode已打开(您可能要在php.ini中改为禁用)',
    'ERR_CHECKSYS_ZLIB'					=> '未找到ZLib支持：SugarCRM可以通过使用zlib压缩提高性能。',
    'ERR_CHECKSYS_ZIP'					=> '未找到ZIP支持：SugarCRM需要ZIP支持来处理压缩文件。',
    'ERR_CHECKSYS_BCMATH'				=> '没有找到BCMATH支持：SugarCRM的任意精度的数学需要BCMATH支持。',
    'ERR_CHECKSYS_HTACCESS'             => '测试.htaccess重写测试失败。一般是表示您没有在sugar目录建立AllowOverride。',
	'ERR_DB_ADMIN'						=> '您提供的数据库管理员用户或密码无效，数据库连接无法建立。请输入有效的用户名密码。(错误：',
    'ERR_DB_ADMIN_MSSQL'                => '提供的数据库管理员用户或密码无效，数据库连接无法建立。请输入有效的用户名密码。',
	'ERR_DB_EXISTS_NOT'					=> '指定的数据库不存在。',
	'ERR_DB_EXISTS_WITH_CONFIG'			=> '数据库配置数据已存在。使用所选数据库进行安装，请重新运行安装并且选择："放弃并重建已存在的SugarCRM表？"  如需升级，请使用升级向导的管理员控制面板。请点击<a href="http://www.sugarforge.org/content/downloads/" target="_new">这里</a>阅读升级文档。',
	'ERR_DB_EXISTS'						=> '提供的数据库名已存在 -- 不能建立相同名字的数据库。',
    'ERR_DB_EXISTS_PROCEED'             => '提供的数据库名已存在。您可以<br>1. 点击返回按钮并选择一个新的数据库名 <br>2. 点击下一步并且继续，但是所有已存在该数据库内的表将被删除。<strong>这意味着您的表和数据将被删除。</strong>',
	'ERR_DB_HOSTNAME'					=> '主机名不能为空。',
	'ERR_DB_INVALID'					=> '选择的数据库类型无效。',
	'ERR_DB_LOGIN_FAILURE'				=> '所提供的数据库主机名、用户名和/或密码无效，无法建立和数据库的连接。请输入有效的主机名、用户名和密码',
	'ERR_DB_LOGIN_FAILURE_MYSQL'		=> '所提供的数据库主机名、用户名和/或密码无效，无法建立和数据库的连接。请输入有效的主机名、用户名和密码',
	'ERR_DB_LOGIN_FAILURE_MSSQL'		=> '所提供的数据库主机名、用户名和/或密码无效，无法建立和数据库的连接。请输入有效的主机名、用户名和密码',
    ...
    'ERR_DB_MSSQL_DB_NAME_INVALID'      => "数据库名的第一位不可采用数字、&#39;#&#39; 或 &#39;@&#39;，同时也不可含有 &#39;\"&#39;, \"&#39;\", &#39;*&#39;, &#39;/&#39;, &#39;\\&#39;, &#39;?&#39;, &#39;:&#39;, &#39;<&#39;, &#39;>&#39;, &#39;&&#39;, &#39;!&#39;, 或 &#39;-&#39;",
    'ERR_DB_OCI8_DB_NAME_INVALID'       => "数据库名称只能包含字母数字字符组和符号 $#39;#$#39;, $#39;_$#39; 或 $#39;$$#39;",
    ...
    'ERR_DBCONF_PASSWORD_MISMATCH'      => '为Sugar数据库用户提供的密码不匹配。请重新输入一致的密码。',
	...
	'ERR_PASSWORD_MISMATCH'				=> '提供的Sugar管理员用户密码不匹配。请重新输入正确的密码。',
	...
	'ERR_PERFORM_NO_TCPIP'				=> '<b>我们无法检测到有效的网络连接。</b> 当您有一个有效连接时，请访问<a href="http://www.sugarcrm.com/home/index.php?option=com_extended_registration&task=register">http://www.sugarcrm.com/home/index.php?option=com_extended_registration&task=register</a> 注册您的SugarCRM。并且让我们了解您的企业怎样使用SugarCRM，让我们确保我们能够继续为您的业务需求提供适用的应用程序。',
	...
	'ERROR_LICENSE_EXPIRED2'			=> "天。请前往位于系统管理界面的<a href=$#39;index.php?action=LicenseSettings&module=Administration$#39;>$#39;\"许可证管理\"</a> 来输入您的新许可证。如果您在许可证过期30天内不输入新的许可证密匙，您将无法登录您的系统。",
	'ERROR_MANIFEST_TYPE'				=> '清单文件中必须指定封装类型。',
	'ERROR_PACKAGE_TYPE'				=> '名单文件指定了一个不能识别的程序包类型',
	'ERROR_VALIDATION_EXPIRED'			=> "错误：您的验证码已过期",
	'ERROR_VALIDATION_EXPIRED2'			=> "天。请前往位于系统管理界面的<a href=$#39;index.php?action=LicenseSettings&module=Administration$#39;>$#39;\"许可证管理\"</a> 来输入您的新验证码。如果您在许可证过期30天内不输入新的验证码，您将无法登录您的系统。",
	...
	'LBL_CHECKSYS_CONFIG_OVERRIDE'		=> '可写入的SugarCRM配置文件(config_override.php)',
...
//	'LBL_NO_THANKS'						=> 'Continue to installer',
    ...
    'advanced_password_new_account_email' => array(
        ...
        'txt_body' =>
'下面是您的帐户用户名和帐户临时密码：<br />用户名 : $contact_user_user_name<br />密码 : $contact_user_user_hash<br /><br />$config_site_url<br /><br />使用上述密码登录后，您将被要求重置您的密码。',
        'name' => '系统生成的密码邮件',
        ),
    'advanced_password_forgot_password_email' => array(
        ...
        'txt_body' =>
'您最近要求了 $contact_user_pwd_last_changed重置您帐户的密码。<br /><br />点击下面的链接来重置您的密码：<br /><br />$contact_user_link_guid',
        'name' => '忘记密码邮件',
        ),
);
